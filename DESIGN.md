...
description: A dim reading-room card catalog for the extension's popup and options surfaces.
colors:
  ground: "#14110e"
  ground-lift: "#1a1713"
  sheet: "#201c17"
...
Cognitive Comfort is a reading room's card catalog rather than a generic dark settings dashboard: an ink-umber ground, dim parchment sheets, hairline rules, one brass interaction accent, Literata catalog lettering, system mono for measured information, and near-crisp paper forms. Surfaces are opaque and quiet. The interface avoids translucent gradient cards, radial glows, glass effects, and equal-weight pills.

The popup leads with the effective state of the current page. Filing a rule settles its sheet with a brass filing mark; pausing stamps the record instead of washing live controls gray. The four-cell media mark uses the same ink-umber, parchment, and brass palette as the panels.
...
### Primary
- **Filing Brass** (`brass`): checked-segment underline, inheritance dot, range thumb and value, links, focus ring, and active ledger mark.
- **Quiet Brass** (`brass-quiet`): hover borders, link underlines, scrollbar hover, shortcut-help rule, and text-selection fill.
- **Brass Wash** (`brass-wash`): low-contrast fill behind checked segments and successful notices; never the only selected-state cue.
...
- **Faint Parchment Ink** (`ink-faint`): metadata, empty subjects, disabled controls, scale labels, and secondary segment details. It is the small-text contrast floor on resting system surfaces.
...
- **Metadata / Note** (11px): supporting notes, segment details, scales, imprint, and stamp. Shortcut assignments are a 12px exception with 0.08em tracking.
...
Spacing has five reusable steps: hair, tight, standard, wide, and section. Sheets use wide horizontal/top padding with standard bottom padding. Shared sheet siblings use standard separation; options increases major sheet separation to section spacing.

The popup shell is exactly `372px` wide. Chrome and Firefox own popup height and scrolling; the popup body only contains overscroll behavior. Safari is the measured exception: a WebKit-only rule pins the popover to a `600px` viewport and scrolls the body to prevent native resize fights, lost momentum, and exposed grey backing. Popup content sits within `16px` side/top and `14px` bottom padding.
...
The system is flat and layered: ground, lifted ground, sheet, raised sheet, and hover sheet establish depth through tone and hairline rules. There are no resting drop shadows. The shared focus ring is a ground separator plus a brass outer ring; filing briefly uses an inset brass edge.
...
### Masthead
Use a flex masthead with a wordmark and optional mono uppercase note; pair it with a standard press action when needed. The masthead ends in a strong rule.
...
**The Live Controls Stay Live Rule.** A paused or otherwise out-of-effect state is expressed by the ledger sentence plus the visible `.stamp` element, never by reducing opacity of live controls or their sheet subject. Controls still work while paused, and newly saved rules remain legible even when not currently in effect. There is no suspended-sheet variant; do not recreate one.
...
### Fields and Segmented Radio Groups
Use a real `<fieldset class="field">` with `<legend class="field-legend">` for grouped choices. Adjacent fields receive `18px` separation. A field note follows its control and is capped at `64ch`; inherited notes use brass only when their text also names the inheritance source.

Use native radio inputs inside labels, with the visible `span` immediately after each input. Checked, hover, focus, and disabled styling depends on `input + span`; do not substitute buttons or class-only state. Add `.is-stacked` for vertical descriptive choices. An inherited checked choice gets a brass dot plus explanatory text naming the source.

### Presses and Links
A press is a bordered serif action with raised-sheet fill; hover uses the hover-sheet fill and a brass-shifted border. Text actions are brass and underlined. Both apply the Forced-Colors Focus Rule.
...
### Fader
The fader head aligns an associated field label and mono value. The range input is followed by an aria-hidden scale and optional field note. The track is a `2px` strong rule, the thumb is a `4px × 18px` brass filing tab, and keyboard focus rings the thumb in WebKit and Gecko.
