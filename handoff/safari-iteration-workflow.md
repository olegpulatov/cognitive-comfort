...
- **Verification Boundary**:
  ...
  - **Playwright WebKit Engine Suite (`tests/e2e/webkit-safari.spec.ts`)**: 11 automated WebKit rendering engine tests verifying WebKit CSS cascade specificity (`:not([data-comfort-revealed="true"])`), YouTube card hover reveal & dynamic preview player mounting reconciliation, YouTube Shorts shelf isolation, multi-layer scrim coordinate penetration, generic video stacks, dynamic node replacement, reveal modes, pointer event preservation, and the popup's Safari popover pin (fixed-height body scroll, dark background, no document scroll).
